...
def test_stitched_ip_shuffle_layer(shuffle_param, datatype, simd):
    ...
    param_str = (
        f"{datatype}_simd{simd}_{hash(str(shuffle_param['in_shape']) + str(shuffle_param['perm']))}"
    )
    ...
    if vivado_stitch_proj_dir and os.path.isdir(vivado_stitch_proj_dir):
        ...
        if synth_results:
            ...
            print(
                f"""
                Resource usage -
                LUT: {results_dict.get('LUT', 'N/A')},
                FF: {results_dict.get('FF', 'N/A')},
                BRAM: {results_dict.get('BRAM', 'N/A')},
                DSP: {results_dict.get('DSP', 'N/A')}
                """
            )
